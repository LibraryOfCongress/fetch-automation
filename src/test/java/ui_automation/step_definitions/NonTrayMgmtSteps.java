...
import ui_automation.pages.NonTrayMgmtPage;
...
public class NonTrayMgmtSteps {

    WebDriver driver = Driver.getInstance().getDriver();
    NonTrayMgmtPage nonTrayMgmt = new NonTrayMgmtPage();
    Helper helper = new Helper();

    WaitHelper wait = new WaitHelper();
    public static final Logger oLog = LogManager.getLogger(NonTrayMgmtSteps.class);


    @Given("user navigates to Non-Trayed Item Management Page")
    public void user_navigates_to_Non_Trayed_Item_Management_Page() {
        Driver.getInstance().getDriver().get(ConfigurationReader.getProperty("ui_config.properties", "nonTrayedItemMgmtURL"));
    }
